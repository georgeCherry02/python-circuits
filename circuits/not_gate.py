...
from .colours import DEAD_COLOUR
...
class NotGate:
    ...
    """
    def __init__(self, input: ConnectionPoint, direction: DIRECTION = "RIGHT"):
        self.direction = direction
        self.input = input
        output_location = input.location + get_dir_vec(direction) * 10
        self.output = ConnectionPoint(output_location, lambda: not self.input.high())

    def draw(self, screen: Surface):
        left_point = self.input.location + (get_dir_vec(get_clockwise_dir(self.direction)) * 4)
        right_point = self.input.location + (get_dir_vec(get_anticlockwise_dir(self.direction)) * 4)
